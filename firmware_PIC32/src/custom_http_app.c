...
void DRV_WIFI_RebootByWebTimerKick(TCPIP_NET_HANDLE currNet, TCPIP_NETWORK_CONFIG * netConfig_ptr);
#endif
...
void TCPIP_HTTP_Print_reboot(HTTP_CONN_HANDLE connHandle)
{
    ...
    {   // valid data
        ...
        //
#if defined(TCPIP_IF_MRF24W)
        DRV_WIFI_RebootByWebTimerKick(httpNetData.currNet,&httpNetData.netConfig);
#else
...
void TCPIP_HTTP_Print_napiecie(HTTP_CONN_HANDLE connHandle)
{
    ...
    char napi[10];
    int i1=Napiecie_int;
    float f1=Napiecie_int-i1;
    int i2=f1*10;
    
    sprintf(napi,"%d.%d",i1,i2); // int to char
    
    Napiecie_str=napi; // char to unit8_t
    
    //Wy?wietlenie na stronie html
    
    TCPIP_TCP_StringPut(sktHTTP, Napiecie_str);  
